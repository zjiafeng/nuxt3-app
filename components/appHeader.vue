              ...
              <img :src="globalConfig.logo" class="vertical-middle" width="120" height="40">
                    ...
                    <ul class="nav-view h-f-100">
                        <li class="nav-item h-f-100" v-for="item in menu" :key="item.key">
                            ...
                            <nuxt-link v-else-if="item.object === 'page'"
                                :to="{ name: 'page-id', state: { id: item.object_id } }" class="first-link">
                                <x-icon :type="item.classes"></x-icon> {{ item.title }}
                            </nuxt-link>
                            <nuxt-link v-else-if="item.object === 'post_tag'"
                                :to="{ name: 'tags-id', state: { type: item.term_id, title: item.name } }"
                                class="first-link">
                                <x-icon :type="item.classes"></x-icon> {{ item.title }}
                            </nuxt-link>
                            ...
                            <div v-if="item.children.length !== 0" class="sub-nav-wrapper">
                                <ul class="sub-nav-view">
                                    <li v-for="child in item.children" :key="child.key" class="sub-item">
                                        <nuxt-link v-if="child.object === 'category'" :to="{
                                            name: 'category-id',
                                            state: { type: child.object_id, title: child.title }
                                        }">
                                            <x-icon :type="child.classes"></x-icon> {{ child.title }}
                                        </nuxt-link>
                                        <nuxt-link v-else-if="child.object === 'page'" :to="{
                                            name: 'page-id',
                                            state: { id: child.object_id }
                                        }">
                                            <x-icon :type="child.classes"></x-icon> {{ child.title }}
                                        </nuxt-link>
                                        <nuxt-link v-else-if="child.object === 'post_tag'"
                                            :to="{ name: 'tags-id', state: { type: child.object_id, title: child.title } }">
                                            <x-icon :type="child.classes"></x-icon> {{ child.title }}
                                        </nuxt-link>
                ...
                <div class="controller">
                    <div class="switch-theme">
                        <input type="checkbox" class="c-switch__input" id="checkbox" v-model="switchValue">
                    ...
                    <div :class="['search-wrapper', isShowSearch && 'is-show']">
                        <div class="search-content">
                            <input v-model="searchText" type="text" placeholder="请输入关键字" @keyup.enter="_search">
...
const mark = ref<boolean>(true)
const switchValue = ref<boolean>(true)
const { globalConfig, menu, menuStatus } = toRefs(useGlobalStore().state)
const height = computed(() => {
    return menuStatus ? `${window.innerHeight}px` : '100%'
})

useHead({
    link: [
        // { rel: 'icon', type: 'image/x-icon', href: globalConfig.favicon }
    ],
    style: [
        // @ts-ignore
        { cssText: globalConfig.globalCss, type: 'text/css' }
    ]
})
const _search = () => {
